...
使用 HttpSession 对象，将数据存入**会话域**就能保持会话状态。
...
参考[会话控制 demo 原始纯净版](https://heavy_code_industry.gitee.io/code_heavy_industry/pro001-javaweb/lecture/chapter09/pro08-cookie-and-session.zip)
...
```html
<div th:if="${#strings.isEmpty(session.sessionHelloAttrName)}">没有从会话域获取到数据</div>
<div th:if="${not #strings.isEmpty(session.sessionHelloAttrName)}">
	<p>
		从会话域读取到：
		<span th:text="${session.sessionHelloAttrName}"></span>
	</p>
</div>
```
